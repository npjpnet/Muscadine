...
const ManageRequestDetail: React.FC = () => {
  ...
  return (
    <DefaultLayout>
      <h1>書類発行申請照会</h1>
      {(request && userData && userMeta && <Layout>
        <Column>
          <h2>申請情報</h2>
          <table>
            <tbody>
          ...
          {(request.status !== 2 && request.status !== 3) && <>
            ...
            <FormSection>
              ...
              <FormItem>
                {request.status === 0 && <>
                  <FormButton onClick={() => handleChangeStatus(1)}>承認</FormButton>
                  <FormButton onClick={() => handleChangeStatus(2)} color="default">却下</FormButton>
                </>}
                {request.status === 1 && <FormButton
                  onClick={() => handleChangeStatus(3)}
                  disabled={!idCardRemarks}>
                  処理済みにする
                </FormButton>}
              </FormItem>
            </FormSection>
