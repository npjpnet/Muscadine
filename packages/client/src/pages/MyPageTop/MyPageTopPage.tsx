...
const MyPageTopPage: React.FC = () => {
  ...
  useEffect(() => {
    const fetchUserAsync = async (): Promise<void> => {
      ...
      const fetchedUserMeta = await getUserMetaByUserIdAsync(user.uid)
      setUserMetaData(fetchedUserMeta)

      const fetchedIDCardHistory = await getIDCardHistoryByUserIdAsync(user.uid)
